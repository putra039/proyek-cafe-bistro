...
if(isset($_POST['konfirmasi'])){
   ...
   $status = $_POST['status'];
   $alasan = $_POST['alasan'];
   if($status === "Pesanan di Konfirmasi"){
   $sql = "UPDATE pembayaran SET status='$status' WHERE id_pemesanan=$id";
   $query = mysqli_query($koneksi,$sql);
   }
   else{
   $sql = "UPDATE pembayaran SET status='$status($alasan)' WHERE id_pemesanan=$id";
   $query = mysqli_query($koneksi,$sql);
   }
   if($query){
...
<?php
          ...
          <div class="col-md-4">
            ...
            <div class="form-group">
                <select  class="custom-select mt-4" name="status" required="" style="margin-bottom:10px ;">
                   <option not selected value="">Check Konfirmasi</option>
                     <option value="Pesanan di Konfirmasi">Konfirmasi</option>
                     <option value="Pesanan di Tolak">Tolak</option>

                     <textarea class="form-control" rows="2" cols="50" placeholder="Alasan Ditolak...." name="alasan" required=""></textarea>
                     <p class="text-right text-danger">*Jika Pesanan Dikonfirmasi Alasan tolak diisi dengan tanda (-) </p>
